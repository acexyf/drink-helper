...
chrome.runtime.onInstalled.addListener(function () {
  ...
  function setClock() {
    ...
    chrome.storage.sync.get(
      ...
      function (data) {
        ...
        chrome.browserAction.setBadgeText({ text: open ? "ON" : "OFF" });
        chrome.browserAction.setBadgeBackgroundColor({
          color: open ? "#63a1ff" : "#f94d4b",
        });
  ...
  function isNowBetween(startTime, endTime) {
    let now = dayjs();
    let prefix = now.format("YYYY-MM-DD");
  ...
  chrome.alarms.onAlarm.addListener(function () {
    ...
    chrome.storage.sync.get(
      ...
      function (data) {
        const { interval, startTime, endTime } = data;
        // 时间到了，判断当前时间是否需要提示
        if (isNowBetween(startTime, endTime)) {
          ...
          let startDate = dayjs(`${prefix} ${startTime}:00`);
          if (now.isAfter(startDate)) {
            nextMinute = startDate.add(1, "d").diff(now, "minute");
          } else {
            nextMinute = startDate.diff(now, "minute");
          }

          chrome.alarms.create({ delayInMinutes: nextMinute });
        }
      }
    );
  });
